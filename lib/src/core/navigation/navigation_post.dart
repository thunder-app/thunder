...
Future<void> navigateToPost(BuildContext context, {Account? account, int? postId, ThunderPost? post, int? highlightedCommentId, Function(ThunderPost post)? onPostUpdated}) async {
  ...
  if (pvm == null) {
    final response = await createPostRepository(effectiveAccount).getPost(postId!);
    pvm = response?.post;
  }
...
Future<ThunderPost> getPostFromComment(ThunderComment comment, Account account) async {
  ...
  final response = await createPostRepository(account).getPost(comment.postId, commentId: comment.id);
