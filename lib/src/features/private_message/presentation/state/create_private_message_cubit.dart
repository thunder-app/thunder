...
class CreatePrivateMessageCubit extends Cubit<CreatePrivateMessageState> {
  ...
  void initialize({ThunderUser? recipient, String? content}) {
    emit(state.copyWith(status: CreatePrivateMessageStatus.initial, recipient: recipient, content: content ?? '', message: null, errorReason: null, privateMessage: null));
  }

  /// Sets the selected recipient.
  void setRecipient(ThunderUser? recipient) {
    emit(state.copyWith(status: CreatePrivateMessageStatus.initial, recipient: recipient, suggestions: const <ThunderUser>[], message: null, errorReason: null));
  }
  ...
  Future<ThunderPrivateMessage?> submit() async {
    ...
    try {
      ...
      emit(state.copyWith(status: CreatePrivateMessageStatus.success, privateMessage: privateMessage, content: '', message: null, errorReason: null));
