...
class InstancePageBloc extends Bloc<InstancePageEvent, InstancePageState> {
  ...
  Future<void> _onGetInstanceCommunities(GetInstanceCommunities event, Emitter<InstancePageState> emit) async {
    ...
    if (shouldSkipFetch(
      currentlyLoading: state.communities.status == InstancePageStatus.loading,
      page: currentPage,
    )) {
      return;
    }

    final previousItems = previousItemsForPage(
      page: currentPage,
      currentItems: state.communities.items,
    );
    ...
    try {
      ...
      final status = hasReachedEnd(
        fetchedCount: communities.length,
      ...
      emit(
        state.copyWith(
          communities: state.communities.copyWith(
            status: status,
            items: [...previousItems, ...communities],
            page: currentPage,
            message: null,
            errorReason: null,
          ),
  ...
  Future<void> _onGetInstanceUsers(GetInstanceUsers event, Emitter<InstancePageState> emit) async {
    ...
    if (shouldSkipFetch(
      currentlyLoading: state.users.status == InstancePageStatus.loading,
      page: currentPage,
    )) {
      return;
    }
    ...
    final previousItems = previousItemsForPage(
      page: currentPage,
      currentItems: state.users.items,
    );
  ...
  Future<void> _onGetInstancePosts(GetInstancePosts event, Emitter<InstancePageState> emit) async {
    ...
    if (shouldSkipFetch(
      currentlyLoading: state.posts.status == InstancePageStatus.loading,
      page: currentPage,
    )) {
      return;
    }
        ...
        onBatchResolved: (resolvedPosts) {
          emit(
            state.copyWith(
              posts: state.posts.copyWith(
                status: InstancePageStatus.loading,
                items: [...previousItems, ...resolvedPosts],
                page: currentPage,
                message: null,
                errorReason: null,
              ),
  ...
  Future<void> _onGetInstanceComments(GetInstanceComments event, Emitter<InstancePageState> emit) async {
    ...
    if (shouldSkipFetch(
      currentlyLoading: state.comments.status == InstancePageStatus.loading,
      page: currentPage,
    )) {
      return;
    }
    ...
    final previousItems = previousItemsForPage(
      page: currentPage,
      currentItems: state.comments.items,
    );
        ...
        onBatchResolved: (resolvedComments) {
          emit(
            state.copyWith(
              comments: state.comments.copyWith(
                status: InstancePageStatus.loading,
                items: [...previousItems, ...resolvedComments],
                page: currentPage,
                message: null,
                errorReason: null,
              ),
