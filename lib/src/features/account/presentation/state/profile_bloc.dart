...
class ProfileBloc extends Bloc<ProfileEvent, ProfileState> {
  ...
  Future<void> _fetchProfileInformation(FetchProfileInformation event, Emitter<ProfileState> emit) async {
    if (account.anonymous) {
      return emit(state.copyWith(status: ProfileStatus.success, reload: event.reload, user: null, subscriptions: [], favorites: [], moderates: [], error: () => null, errorReason: () => null));
    }

    try {
      emit(state.copyWith(status: ProfileStatus.loading, user: null, moderates: [], reload: event.reload, error: () => null, errorReason: () => null));
      ...
      if (isSameUser(user: user, account: account)) {
        return emit(state.copyWith(status: ProfileStatus.success, user: () => user, moderates: moderates, reload: event.reload, error: () => null, errorReason: () => null));
      }

      return emit(state.copyWith(status: ProfileStatus.success, user: null, moderates: [], reload: event.reload, error: () => null, errorReason: () => null));
    } catch (e) {
  ...
  Future<void> _fetchProfileSettings(FetchProfileSettings event, Emitter<ProfileState> emit) async {
    if (account.anonymous) {
      return emit(state.copyWith(status: ProfileStatus.success, error: () => null, errorReason: () => null));
    }

    try {
      emit(state.copyWith(status: ProfileStatus.loading, error: () => null, errorReason: () => null));

      // Refresh the site information, which includes the user's settings
      final response = await instanceRepository!.info();

      return emit(state.copyWith(status: ProfileStatus.success, siteResponse: () => response, error: () => null, errorReason: () => null));
    } catch (e) {
  ...
  Future<void> _fetchProfileSubscriptions(FetchProfileSubscriptions event, Emitter<ProfileState> emit) async {
    if (account.anonymous) {
      return emit(state.copyWith(status: ProfileStatus.success, reload: event.reload, subscriptions: [], favorites: [], error: () => null, errorReason: () => null));
    }

    try {
      emit(state.copyWith(status: ProfileStatus.loading, reload: event.reload, error: () => null, errorReason: () => null));
      final subscriptions = await accountRepository!.subscriptions();
      emit(state.copyWith(status: ProfileStatus.success, reload: event.reload, subscriptions: subscriptions, error: () => null, errorReason: () => null));
  ...
  Future<void> _fetchProfileFavorites(FetchProfileFavorites event, Emitter<ProfileState> emit) async {
    if (account.anonymous) {
      return emit(state.copyWith(status: ProfileStatus.success, reload: event.reload, favorites: [], error: () => null, errorReason: () => null));
    }
    ...
    try {
      ...
      final communities = filterFavorites(subscriptions: state.subscriptions, favorites: favorites);

      return emit(state.copyWith(status: ProfileStatus.success, reload: event.reload, favorites: communities, error: () => null, errorReason: () => null));
    } catch (e) {
