...
class ModlogFeedPageAppBar extends StatelessWidget {
  const ModlogFeedPageAppBar(
      {super.key, required this.showAppBarTitle, required this.lemmyClient});
  ...
  Widget build(BuildContext context) {
    ...
    return SliverAppBar(
      ...
      title: ModlogFeedAppBarTitle(
          visible: showAppBarTitle, lemmyClient: lemmyClient),
      leading: IconButton(
          ...
          onPressed: () {
            ...
            showModalBottomSheet<void>(
              ...
              builder: (builderContext) => ModlogActionTypePicker(
                title: l10n.filters,
                onSelect: (selected) async => context.read<ModlogBloc>().add(
                    ModlogFeedChangeFilterTypeEvent(
                        modlogActionType: selected.payload)),
                previouslySelected:
                    context.read<ModlogBloc>().state.modlogActionType,
              ),
            );
          },
        ),
...
class ModlogFeedAppBarTitle extends StatelessWidget {
  const ModlogFeedAppBarTitle(
      {super.key, this.visible = true, required this.lemmyClient});
      ...
      child: ListTile(
        ...
        subtitle: feedState.fullCommunityView != null
            ? CommunityFullNameWidget(
                ...
                fetchInstanceNameFromUrl(feedState
                    .fullCommunityView!.communityView.community.actorId),
              )
