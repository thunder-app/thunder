...
class _PostSubviewState extends State<PostSubview> {
  ...
  Widget build(BuildContext context) {
    ...
    feedBloc = widget.feedBloc ?? FeedBloc(lemmyClient: LemmyClient.instance);

    return BlocListener<FeedBloc, FeedState>(
      bloc: feedBloc,
      listener: (BuildContext context, FeedState state) {
        if (state.status == FeedStatus.success) {
          final PostViewMedia? editedPostViewMedia = state.postViewMedias.firstWhereOrNull((pvm) => pvm.postView.post.id == postView.post.id);
          if (editedPostViewMedia != null) {
            setState(() {
              postViewMedia = editedPostViewMedia;
            });
          }
        }
      },
