...
class PostBloc extends Bloc<PostEvent, PostState> {
  PostBloc() : super(const PostState()) {
    ...
    on<UpdateCommentEvent>(
      _updateCommentEvent,
      transformer:
          throttleDroppable(Duration.zero), // Don't give a throttle on update
    );
  ...
  Future<void> _getPostEvent(GetPostEvent event, emit) async {
    ...
    try {
      ...
      CommentSortType defaultSortType = CommentSortType.values.byName(prefs
              .getString(LocalSettings.defaultCommentSortType.name)
              ?.toLowerCase() ??
          DEFAULT_COMMENT_SORT_TYPE.name);
      defaultSortType =
          LemmyClient.instance.supportsCommentSortType(defaultSortType)
              ? defaultSortType
        ...
        try {
          ...
          if (postId != null) {
            getPostResponse = await lemmy
                .run(GetPost(id: postId, auth: account?.jwt))
                .timeout(timeout, onTimeout: () {
              throw Exception(
                  AppLocalizations.of(GlobalContext.context)!.timeoutComments);
            });
          }
          ...
          if (getPostResponse != null) {
            ...
            List<PostViewMedia> posts =
                await parsePostViews([getPostResponse.postView]);
          ...
          if (moderators == null && postView != null) {
            try {
              moderators = (await lemmy
                      .run(GetCommunity(
                          id: postView.postView.community.id,
                          auth: account?.jwt))
                      .timeout(timeout, onTimeout: () {
                throw Exception();
              }))
          ...
          List<CommentViewTree> commentTree =
              buildCommentViewTree(getCommentsResponse.comments);
          ...
          return emit(
            state.copyWith(
                ...
                commentPage: state.commentPage +
                    (event.selectedCommentId == null ? 1 : 0),
                commentResponseMap: responseMap,
                commentCount: getCommentsResponse.comments.length,
                hasReachedCommentEnd: getCommentsResponse.comments.isEmpty ||
                    getCommentsResponse.comments.length < commentLimit,
                communityId: postView?.postView.post.communityId,
                sortType: sortType,
                selectedCommentId: event.selectedCommentId,
                selectedCommentPath: event.selectedCommentPath,
      ...
      emit(state.copyWith(
          ...
          errorMessage: getExceptionErrorMessage(exception)));
    } catch (e) {
      emit(state.copyWith(
    ...
    try {
      ...
      while (attemptCount < 2) {
        try {
          ...
          if (event.reset || event.viewAllCommentsRefresh) {
            if (event.viewAllCommentsRefresh) {
              emit(state.copyWith(
                  ...
                  viewAllCommentsRefresh: true,
                  sortType: sortType));
            } else {
              emit(state.copyWith(
                  status: PostStatus.loading, sortType: sortType));
            }
            ...
            CommentNode comments =
                buildCommentTree(getCommentsResponse.comments);
            ...
            return emit(
              state.copyWith(
                  ...
                  hasReachedCommentEnd: getCommentsResponse.comments.isEmpty ||
                      commentTree.length < commentLimit,
                  sortType: sortType),
            );
          }

          // Prevent duplicate requests if we're done fetching comments
          if (state.commentCount >= state.postView!.postView.counts.comments ||
              (event.commentParentId == null && state.hasReachedCommentEnd)) {
            if (!state.hasReachedCommentEnd &&
                state.commentCount >=
                    state.postView!.postView.counts.comments) {
              emit(state.copyWith(
                  status: state.status, hasReachedCommentEnd: true));
            }
          ...
          if (event.commentParentId != null) {
            final bool anyDirectChildren = getCommentsResponse.comments.any(
                (commentView) => commentView.comment.path.contains(
                    '${event.commentParentId}.${commentView.comment.id}'));
            if (!anyDirectChildren) {
              throw Exception(AppLocalizations.of(GlobalContext.context)!
                  .unableToLoadReplies);
            }
          }

          // Combine all of the previous comments list
          List<CommentView> fullCommentResponseList =
              List.from(state.commentResponseMap.values)
                ..addAll(getCommentsResponse.comments);

          for (CommentView comment in getCommentsResponse.comments) {
            state.commentResponseMap[comment.comment.id] = comment;
          }
          // Build the tree view from the flattened comments
          List<CommentViewTree> commentViewTree =
              buildCommentViewTree(fullCommentResponseList);
          ...
          return emit(state.copyWith(
            ...
            commentPage:
                event.commentParentId != null ? 1 : state.commentPage + 1,
            commentCount: state.commentResponseMap.length,
            hasReachedCommentEnd: event.commentParentId != null ||
                (getCommentsResponse.comments.isEmpty ||
                    state.commentCount == state.commentResponseMap.length),
          ));
        } catch (e) {
          exception = e;
          attemptCount++;
      ...
      if (is50xError(exception.toString()) != null) {
        emit(state.copyWith(
            status: PostStatus.failure,
            errorMessage: AppLocalizations.of(GlobalContext.context)!
                .serverErrorComments('${is50xError(exception.toString())}')));
      } else {
        ...
        emit(state.copyWith(
            status: PostStatus.failure, errorMessage: exception.toString()));
      }
    } catch (e) {
      emit(state.copyWith(
          status: PostStatus.failure, errorMessage: e.toString()));
  ...
  Future<void> _votePostEvent(
      VotePostEvent event, Emitter<PostState> emit) async {
    try {
      emit(state.copyWith(
          status: PostStatus.refreshing,
          selectedCommentId: state.selectedCommentId,
      ...
      PostView updatedPostView =
          optimisticallyVotePost(state.postView!, event.score);
      ...
      PostView postView = await votePost(event.postId, event.score)
        ...
        throw Exception(
            AppLocalizations.of(GlobalContext.context)!.timeoutVotingPost);
      });
  ...
  Future<void> _savePostEvent(
    ...
    try {
      ...
      PostView postView = await savePost(event.postId, event.save)
          .timeout(timeout, onTimeout: () {
        throw Exception(
            AppLocalizations.of(GlobalContext.context)!.timeoutSavingPost);
      });
  ...
  Future<void> _commentActionEvent(
      CommentActionEvent event, Emitter<PostState> emit) async {
    emit(state.copyWith(
        status: PostStatus.refreshing,
        selectedCommentId: state.selectedCommentId,
        selectedCommentPath: state.selectedCommentPath));
    ...
    CommentNode? existingCommentNode = CommentNode.findCommentNode(
        state.commentNodes!, event.commentId.toString());
    if (existingCommentNode == null)
      return emit(state.copyWith(status: PostStatus.failure));

    List<String> commentPath =
        existingCommentNode.commentView!.comment.path.split('.');
    String parentId = commentPath[commentPath.length - 2];

    switch (event.action) {
      case CommentAction.vote:
        try {
          CommentNode newCommentNode = CommentNode(
              commentView: optimisticallyVoteComment(
                  existingCommentNode.commentView!, event.value),
              replies: existingCommentNode.replies);
          CommentNode.insertCommentNode(
              state.commentNodes!, parentId, newCommentNode);
          ...
          await voteComment(event.commentId, event.value).timeout(timeout,
              onTimeout: () {
            // Restore the original comment if vote fails
            CommentNode.insertCommentNode(
                state.commentNodes!, parentId, existingCommentNode);
            throw Exception(AppLocalizations.of(GlobalContext.context)!
      ...
      case CommentAction.save:
        try {
          CommentNode newCommentNode = CommentNode(
              commentView: optimisticallySaveComment(
                  existingCommentNode.commentView!, event.value),
              replies: existingCommentNode.replies);
          CommentNode.insertCommentNode(
              state.commentNodes!, parentId, newCommentNode);
          ...
          await saveComment(event.commentId, event.value).timeout(timeout,
              onTimeout: () {
            // Restore the original comment if vote fails
            CommentNode.insertCommentNode(
                state.commentNodes!, parentId, existingCommentNode);
      ...
      case CommentAction.delete:
        try {
          CommentNode newCommentNode = CommentNode(
              commentView: optimisticallyDeleteComment(
                  existingCommentNode.commentView!, event.value),
              replies: existingCommentNode.replies);
          CommentNode.insertCommentNode(
              state.commentNodes!, parentId, newCommentNode);
          ...
          await deleteComment(event.commentId, event.value).timeout(timeout,
              onTimeout: () {
            // Restore the original comment if vote fails
            CommentNode.insertCommentNode(
                state.commentNodes!, parentId, existingCommentNode);
  ...
  Future<void> _commentItemUpdatedEvent(
      CommentItemUpdatedEvent event, Emitter<PostState> emit) async {
    if (state.commentNodes == null)
      return emit(state.copyWith(status: PostStatus.failure));
    emit(state.copyWith(status: PostStatus.refreshing));

    CommentNode? commentNode = CommentNode.findCommentNode(
        state.commentNodes!, event.commentView.comment.id.toString());
    List<String> commentPath = event.commentView.comment.path.split('.');
    String parentId = commentPath[commentPath.length - 2];

    if (commentNode == null) {
      // This is most likely a new comment
      CommentNode.insertCommentNode(state.commentNodes!, parentId,
          CommentNode(commentView: event.commentView, replies: []));
    ...
    CommentNode.insertCommentNode(
        ...
        parentId,
        CommentNode(
            commentView: event.commentView, replies: commentNode.replies));
  ...
  Future<void> _voteCommentEvent(
      VoteCommentEvent event, Emitter<PostState> emit) async {
    try {
      emit(state.copyWith(
          status: PostStatus.refreshing,
          selectedCommentId: state.selectedCommentId,
      ...
      List<int> commentIndexes = findCommentIndexesFromCommentViewTree(
          state.comments, event.commentId);
      CommentViewTree currentTree =
          state.comments[commentIndexes[0]]; // Get the initial CommentViewTree

      for (int i = 1; i < commentIndexes.length; i++) {
        currentTree = currentTree
            .replies[commentIndexes[i]]; // Traverse to the next CommentViewTree
      }
      ...
      CommentView updatedCommentView =
          optimisticallyVoteComment(currentTree.commentView!, event.score);
      ...
      CommentView commentView = await voteComment(event.commentId, event.score)
          .timeout(timeout, onTimeout: () {
        currentTree.commentView =
            originalCommentView; // Reset this on exception
        throw Exception(
            AppLocalizations.of(GlobalContext.context)!.timeoutUpvoteComment);
      });
  ...
  Future<void> _saveCommentEvent(
      SaveCommentEvent event, Emitter<PostState> emit) async {
    try {
      emit(state.copyWith(
          status: PostStatus.refreshing,
          selectedCommentId: state.selectedCommentId,
      ...
      CommentView commentView = await saveComment(event.commentId, event.save)
          .timeout(timeout, onTimeout: () {
        throw Exception(
            AppLocalizations.of(GlobalContext.context)!.timeoutSaveComment);
      });
  ...
  Future<void> _deleteCommentEvent(
    ...
    try {
      ...
      CommentResponse deletedComment = await lemmy.run(DeleteComment(
  ...
  Future<void> _updateCommentEvent(
    ...
    try {
      ...
      List<CommentViewTree> updatedComments =
          insertNewComment(state.comments, event.commentView);
  ...
  Future<void> _reportCommentEvent(
      ReportCommentEvent event, Emitter<PostState> emit) async {
    try {
      emit(state.copyWith(
          status: PostStatus.refreshing,
          moddingCommentId: event.commentId,
      ...
      await lemmy.run(CreateCommentReport(
          commentId: event.commentId,
  ...
  Future<void> _navigateCommentEvent(
      NavigateCommentEvent event, Emitter<PostState> emit) async {
  ...
  Future<void> _startCommentSearchEvent(
      StartCommentSearchEvent event, Emitter<PostState> emit) async {
    ...
    return emit(state.copyWith(
      ...
      navigateCommentIndex: parentComment == null
          ? null
          : state.comments.indexOf(state.comments.firstWhere(
                  (c) => c.commentView?.comment.id == parentComment.id)) +
              1,
      navigateCommentId: state.navigateCommentId + 1,
    ));
  }

  Future<void> _continueCommentSearchEvent(
      ContinueCommentSearchEvent event, Emitter<PostState> emit) async {
    ...
    Comment? existingSelectedComment = state.commentMatches!
        .firstWhereOrNull((c) => c.id == state.newlyCreatedCommentId);
    if (state.newlyCreatedCommentId != null &&
        existingSelectedComment != null) {
  ...
  Future<void> _endCommentSearchEvent(
      EndCommentSearchEvent event, Emitter<PostState> emit) async {
    return emit(state.copyWith(
      status: PostStatus.success,
  ...
  Comment? findParent(Comment comment) {
    ...
    bool childrenContains(CommentViewTree commentViewTree, Comment comment) {
      if (commentViewTree.replies.firstWhereOrNull(
              (cvt) => cvt.commentView?.comment.id == comment.id) !=
          null) {
        return true;
      } else {
    ...
    for (CommentViewTree commentViewTree in state.comments) {
      if (commentViewTree.commentView!.comment.id == comment.id ||
          childrenContains(commentViewTree, comment)) {
