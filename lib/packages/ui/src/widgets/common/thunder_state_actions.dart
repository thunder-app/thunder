...
class ThunderStateActions extends StatelessWidget {
  const ThunderStateActions({super.key, required this.actions});
      ...
      children: [
        for (int i = 0; i < actions.length; i++) ...[
          SizedBox(
            ...
            child: _ThunderStateActionButton(action: actions[i], primary: actions[i].primary || i == 0),
          ),
...
class _ThunderStateActionButton extends StatelessWidget {
  const _ThunderStateActionButton({required this.action, required this.primary});
  ...
  Widget build(BuildContext context) {
    ...
    if (primary) {
      return ElevatedButton(onPressed: action.loading ? null : action.onPressed, child: child);
    }

    return TextButton(onPressed: action.loading ? null : action.onPressed, child: child);
  }
}
