...
class ThunderSkeletonBarSpec {
  const ThunderSkeletonBarSpec({required this.width, this.height = 10.0, this.opacity = 0.25, this.padding = EdgeInsets.zero});
...
class ThunderSkeletonPlaceholder extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [for (final bar in bars) ThunderSkeletonBar(width: bar.width, height: bar.height, opacity: bar.opacity, padding: bar.padding)],
    );
  }
}
