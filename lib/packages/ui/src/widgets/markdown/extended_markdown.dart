...
abstract class ExtendedMarkdownWidget extends MarkdownWidget {
  ...
  // Delegate through a public method so State can invoke the child builder
  // without directly accessing MarkdownWidget's protected member.
  Widget buildWithChildren(BuildContext context, List<Widget>? children) {
    return build(context, children);
  }
...
class _MarkdownWidgetState extends State<ExtendedMarkdownWidget> implements MarkdownBuilderDelegate {
  ...
  @override
  Widget build(BuildContext context) => widget.buildWithChildren(context, _children);
}
