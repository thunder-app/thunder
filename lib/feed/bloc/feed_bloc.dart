...
class FeedBloc extends Bloc<FeedEvent, FeedState> {
  ...
  Future<void> _onFeedHidePostsFromView(
      FeedHidePostsFromViewEvent event, Emitter<FeedState> emit) async {
    emit(state.copyWith(status: FeedStatus.fetching));

    List<PostViewMedia> postViewMedias = List.from(state.postViewMedias);
    postViewMedias.removeWhere((PostViewMedia postViewMedia) =>
        event.postIds.contains(postViewMedia.postView.post.id));

    emit(state.copyWith(
        status: FeedStatus.success, postViewMedias: postViewMedias));
  }

  /// Handles dismissing read posts from the feed
  Future<void> _onFeedDismissRead(
      FeedDismissReadEvent event, Emitter<FeedState> emit) async {
    emit(state.copyWith(
        status: FeedStatus.success, dismissReadId: state.dismissReadId + 1));
  }

  /// Handles dismissing read posts from the feed
  Future<void> _onFeedDismissBlocked(
      FeedDismissBlockedEvent event, Emitter<FeedState> emit) async {
    emit(state.copyWith(
        status: FeedStatus.success,
        dismissBlockedUserId: event.userId,
        dismissBlockedCommunityId: event.communityId));
  }

  /// Handles dismissing read posts from the feed
  Future<void> _onFeedDismissHiddenPost(
      FeedDismissHiddenPostEvent event, Emitter<FeedState> emit) async {
    emit(state.copyWith(
        status: FeedStatus.success, dismissHiddenPostId: event.postId));
  }

  /// Handles scrolling to top of the feed
  Future<void> _onFeedScrollToTop(
      ScrollToTopEvent event, Emitter<FeedState> emit) async {
    emit(state.copyWith(
        status: FeedStatus.success, scrollId: state.scrollId + 1));
  }

  /// Handles clearing any messages from the state
  Future<void> _onFeedClearMessage(
      FeedClearMessageEvent event, Emitter<FeedState> emit) async {
    emit(state.copyWith(
        status: state.status == FeedStatus.failureLoadingCommunity ||
  ...
  Future<void> _onFeedItemActioned(
      FeedItemActionedEvent event, Emitter<FeedState> emit) async {
    assert(!(event.postViewMedia == null &&
        event.postId == null &&
        event.postIds == null));
    emit(state.copyWith(status: FeedStatus.fetching));
    ...
    switch (event.postAction) {
      case PostAction.vote:
        ...
        int existingPostViewMediaIndex = state.postViewMedias.indexWhere(
            (PostViewMedia postViewMedia) =>
                postViewMedia.postView.post.id == event.postId);
        ...
        try {
          PostView updatedPostView =
              optimisticallyVotePost(postViewMedia, event.value);
          state.postViewMedias[existingPostViewMediaIndex].postView =
              updatedPostView;
          ...
          PostView postView =
              await votePost(originalPostView.post.id, event.value);
      ...
      case PostAction.save:
        ...
        try {
          PostView updatedPostView =
              optimisticallySavePost(postViewMedia, event.value);
          state.postViewMedias[existingPostViewMediaIndex].postView =
              updatedPostView;
          ...
          PostView postView =
              await savePost(originalPostView.post.id, event.value);
          state.postViewMedias[existingPostViewMediaIndex].postView = postView;
      ...
      case PostAction.read:
        ...
        int existingPostViewMediaIndex = state.postViewMedias.indexWhere(
            (PostViewMedia postViewMedia) =>
                postViewMedia.postView.post.id == event.postId);
        if (existingPostViewMediaIndex == -1)
          return emit(state.copyWith(status: FeedStatus.failure));
        ...
        try {
          PostView updatedPostView =
              optimisticallyReadPost(postViewMedia, event.value);
          state.postViewMedias[existingPostViewMediaIndex].postView =
              updatedPostView;
          ...
          bool success =
              await markPostAsRead(originalPostView.post.id, event.value);
      ...
      case PostAction.multiRead:
        ...
        if (eventPostIds.isNotEmpty) {
          ...
          for (int i = 0; i < state.postViewMedias.length; i++) {
            if (eventPostIds
                .contains(state.postViewMedias[i].postView.post.id)) {
          ...
          try {
            for (int i = 0; i < existingPostViewMediaIndexes.length; i++) {
              PostView updatedPostView =
                  optimisticallyReadPost(postViewMedias[i], event.value);
              state.postViewMedias[existingPostViewMediaIndexes[i]].postView =
                  updatedPostView;
            }
            ...
            for (int i = 0; i < failed.length; i++) {
              state.postViewMedias[existingPostViewMediaIndexes[failed[i]]]
                  .postView = originalPostViews[failed[i]];
            }
            return emit(state.copyWith(status: FeedStatus.failure));
          } catch (e) {
            // Restore the original post contents
      ...
      case PostAction.hide:
        ...
        try {
          PostView updatedPostView =
              optimisticallyHidePost(postViewMedia, event.value);
          state.postViewMedias[existingPostViewMediaIndex].postView =
              updatedPostView;
          ...
          bool success =
              await markPostAsHidden(originalPostView.post.id, event.value);
          if (success) return emit(state.copyWith(status: FeedStatus.success));
      ...
      case PostAction.delete:
        ...
        try {
          PostView updatedPostView =
              optimisticallyDeletePost(postViewMedia.postView, event.value);
          state.postViewMedias[existingPostViewMediaIndex].postView =
              updatedPostView;
          ...
          bool success =
              await deletePost(originalPostView.post.id, event.value);
          if (success) return emit(state.copyWith(status: FeedStatus.success));
      ...
      case PostAction.lock:
        ...
        try {
          PostView updatedPostView =
              optimisticallyLockPost(postViewMedia.postView, event.value);
          state.postViewMedias[existingPostViewMediaIndex].postView =
              updatedPostView;
      ...
      case PostAction.pinCommunity:
        ...
        try {
          PostView updatedPostView = optimisticallyPinPostToCommunity(
              postViewMedia.postView, event.value);
          state.postViewMedias[existingPostViewMediaIndex].postView =
              updatedPostView;
          ...
          bool success =
              await pinPostToCommunity(originalPostView.post.id, event.value);
          if (success) return emit(state.copyWith(status: FeedStatus.success));
      ...
      case PostAction.remove:
        ...
        try {
          PostView updatedPostView = optimisticallyRemovePost(
              postViewMedia.postView, event.value['remove']);
          state.postViewMedias[existingPostViewMediaIndex].postView =
              updatedPostView;
          ...
          bool success = await removePost(originalPostView.post.id,
              event.value['remove'], event.value['reason']);
          if (success) return emit(state.copyWith(status: FeedStatus.success));
      ...
      default:
        emit(state.copyWith(
            status: FeedStatus.failure, message: 'Action is not supported'));
        break;
    }
  ...
  /// Handles updating a given item within the feed
  Future<void> _onFeedItemUpdated(
      FeedItemUpdatedEvent event, Emitter<FeedState> emit) async {
    emit(state.copyWith(status: FeedStatus.fetching));

    // TODO: Add support for updating comments (for user profile)
    for (final (index, postViewMedia) in state.postViewMedias.indexed) {
      if (postViewMedia.postView.post.id ==
    ...
    emit(state.copyWith(
        status: FeedStatus.success, postViewMedias: state.postViewMedias));
  }

  /// Handles updating information about a community
  Future<void> _onFeedCommunityViewUpdated(
      FeedCommunityViewUpdatedEvent event, Emitter<FeedState> emit) async {
    emit(state.copyWith(status: FeedStatus.fetching));

    GetCommunityResponse? updatedFullCommunityView =
        state.fullCommunityView?.copyWith(communityView: event.communityView);

    emit(state.copyWith(
        status: FeedStatus.success,
        fullCommunityView: updatedFullCommunityView));
  }

  /// Resets the FeedState to its initial state
  Future<void> _onResetFeed(
      ResetFeedEvent event, Emitter<FeedState> emit) async {
  ...
  Future<void> _onFeedChangeSortType(
      FeedChangeSortTypeEvent event, Emitter<FeedState> emit) async {
  ...
  Future<void> _onFeedFetched(
      FeedFetchedEvent event, Emitter<FeedState> emit) async {
    ...
    if (event.reset && event.feedType == FeedType.general)
      assert(event.postListingType != null);
    ...
    if (event.reset) {
      ...
      switch (event.feedType) {
        case FeedType.community:
          ...
          try {
            fullCommunityView = await fetchCommunityInformation(
                id: event.communityId, name: event.communityName);
          } catch (e) {
        ...
        case FeedType.user:
          ...
          try {
            ...
            return emit(state.copyWith(
              ...
              message:
                  getExceptionErrorMessage(e, additionalInfo: event.username),
              feedType: event.feedType,
            ));
          }
          break;
  ...
  Future<void> _onCreatePost(
    ...
    try {
      ...
      List<PostViewMedia> updatedPostViewMedias =
          List.from(state.postViewMedias);
      updatedPostViewMedias.insert(0, formattedPost[0]);

      emit(state.copyWith(
          status: FeedStatus.success, postViewMedias: updatedPostViewMedias));
    } catch (e) {
      return emit(
          state.copyWith(status: FeedStatus.failure, message: e.toString()));
    }
  }

  Future<void> _onPopulatePosts(
      PopulatePostsEvent event, Emitter<FeedState> emit) async {
