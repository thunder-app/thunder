...
Future<BlockCommunityResponse> blockCommunity(
    int communityId, bool block) async {
...
Future<GetCommunityResponse> fetchCommunityInformation(
...
Future<void> toggleFavoriteCommunity(
    BuildContext context, Community community, bool isFavorite) async {
  if (isFavorite) {
    await Favorite.deleteFavorite(communityId: community.id);
    if (context.mounted)
      context.read<AccountBloc>().add(const RefreshAccountInformation());
    return;
  }
