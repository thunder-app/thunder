...
class ThunderBloc extends Bloc<ThunderEvent, ThunderState> {
  ...
  Future<void> _userPreferencesChangeEvent(
    ...
    try {
      ...
      NotificationType inboxNotificationType = NotificationType.values.byName(
              prefs.getString(LocalSettings.inboxNotificationType.name) ??
                  NotificationType.none.name) ??
          NotificationType.none;
      String? appLanguageCode =
          prefs.getString(LocalSettings.appLanguageCode.name) ?? 'en';
      FullNameSeparator userSeparator = FullNameSeparator.values.byName(
          prefs.getString(LocalSettings.userFormat.name) ??
              FullNameSeparator.at.name);
      NameThickness userFullNameUserNameThickness = NameThickness.values.byName(
          prefs.getString(LocalSettings.userFullNameUserNameThickness.name) ??
              NameThickness.normal.name);
      NameColor userFullNameUserNameColor = NameColor.fromString(
          color:
              prefs.getString(LocalSettings.userFullNameUserNameColor.name) ??
                  NameColor.defaultColor);
      NameThickness userFullNameInstanceNameThickness = NameThickness.values
          .byName(prefs.getString(
                  LocalSettings.userFullNameInstanceNameThickness.name) ??
              NameThickness.light.name);
      NameColor userFullNameInstanceNameColor = NameColor.fromString(
          color: prefs.getString(
                  LocalSettings.userFullNameInstanceNameColor.name) ??
              NameColor.defaultColor);
      FullNameSeparator communitySeparator = FullNameSeparator.values.byName(
      ...
      NameThickness communityFullNameCommunityNameThickness =
          NameThickness.values.byName(prefs.getString(
                  LocalSettings.communityFullNameCommunityNameThickness.name) ??
              NameThickness.normal.name);
      NameColor communityFullNameCommunityNameColor = NameColor.fromString(
          color: prefs.getString(
      ...
      NameThickness communityFullNameInstanceNameThickness =
          NameThickness.values.byName(prefs.getString(
              ...
              NameThickness.light.name);
      NameColor communityFullNameInstanceNameColor = NameColor.fromString(
          color: prefs.getString(
                  LocalSettings.communityFullNameInstanceNameColor.name) ??
              NameColor.defaultColor);
      ImageCachingMode imageCachingMode = ImageCachingMode.values.byName(
      ...
      BrowserMode browserMode = BrowserMode.values.byName(
      ...
      bool scoreCounters =
          prefs.getBool(LocalSettings.scoreCounters.name) ?? false;
      bool dimReadPosts =
          prefs.getBool(LocalSettings.dimReadPosts.name) ?? true;
      bool showFullPostDate =
          prefs.getBool(LocalSettings.showFullPostDate.name) ?? false;
      DateFormat dateFormat = DateFormat(
          prefs.getString(LocalSettings.dateFormat.name) ??
              DateFormat.yMMMMd(Intl.systemLocale).add_jm().pattern);
      FeedCardDividerThickness feedCardDividerThickness =
          FeedCardDividerThickness.values.byName(
              prefs.getString(LocalSettings.feedCardDividerThickness.name) ??
                  FeedCardDividerThickness.compact.name);
      Color feedCardDividerColor = Color(
          prefs.getInt(LocalSettings.feedCardDividerColor.name) ??
              Colors.transparent.value);
      List<PostCardMetadataItem> compactPostCardMetadataItems = prefs
      ...
      List<PostCardMetadataItem> cardPostCardMetadataItems = prefs
      ...
      // Post body settings
      bool showCrossPosts =
          prefs.getBool(LocalSettings.showCrossPosts.name) ?? true;
      PostBodyViewType postBodyViewType = PostBodyViewType.values.byName(
      ...
      FontScale contentFontSizeScale = FontScale.values.byName(
          prefs.getString(LocalSettings.contentFontSizeScale.name) ??
              FontScale.base.name);
      FontScale commentFontSizeScale = FontScale.values.byName(
          prefs.getString(LocalSettings.commentFontSizeScale.name) ??
              FontScale.base.name);
      FontScale metadataFontSizeScale = FontScale.values.byName(
      ...
      bool enableFullScreenSwipeNavigationGesture = prefs.getBool(
              LocalSettings.enableFullScreenSwipeNavigationGesture.name) ??
          true;

      /// -------------------------- FAB Related Settings --------------------------
      bool enableFeedsFab =
          prefs.getBool(LocalSettings.enableFeedsFab.name) ?? true;
      bool enablePostsFab =
          prefs.getBool(LocalSettings.enablePostsFab.name) ?? true;
      ...
      bool postFabEnableBackToTop =
          prefs.getBool(LocalSettings.postFabEnableBackToTop.name) ?? true;
      bool postFabEnableChangeSort =
          prefs.getBool(LocalSettings.postFabEnableChangeSort.name) ?? true;
      bool postFabEnableReplyToPost =
          prefs.getBool(LocalSettings.postFabEnableReplyToPost.name) ?? true;
      bool postFabEnableRefresh =
          prefs.getBool(LocalSettings.postFabEnableRefresh.name) ?? true;
      bool postFabEnableSearch =
          prefs.getBool(LocalSettings.postFabEnableSearch.name) ?? true;

      FeedFabAction feedFabSinglePressAction = FeedFabAction.values.byName(
          prefs.getString(LocalSettings.feedFabSinglePressAction.name) ??
              FeedFabAction.newPost.name);
      FeedFabAction feedFabLongPressAction = FeedFabAction.values.byName(
          prefs.getString(LocalSettings.feedFabLongPressAction.name) ??
              FeedFabAction.openFab.name);
      PostFabAction postFabSinglePressAction = PostFabAction.values.byName(
          prefs.getString(LocalSettings.postFabSinglePressAction.name) ??
              PostFabAction.replyToPost.name);
      PostFabAction postFabLongPressAction = PostFabAction.values.byName(
          prefs.getString(LocalSettings.postFabLongPressAction.name) ??
              PostFabAction.openFab.name);
      ...
      bool videoAutoFullscreen =
          prefs.getBool(LocalSettings.videoAutoFullscreen.name) ?? false;
      bool videoAutoLoop =
          prefs.getBool(LocalSettings.videoAutoLoop.name) ?? false;
      bool videoAutoMute =
          prefs.getBool(LocalSettings.videoAutoMute.name) ?? true;
      VideoAutoPlay videoAutoPlay = VideoAutoPlay.values.byName(
          prefs.getString(LocalSettings.videoAutoPlay.name) ??
              VideoAutoPlay.never.name);
      VideoPlayBackSpeed videoDefaultPlaybackSpeed = VideoPlayBackSpeed.values
          .byName(
              prefs.getString(LocalSettings.videoDefaultPlaybackSpeed.name) ??
                  VideoPlayBackSpeed.normal.name);

      String currentAnonymousInstance =
          prefs.getString(LocalSettings.currentAnonymousInstance.name) ??
              'lemmy.ml';
  ...
  void _onSetCurrentAnonymousInstance(
      OnSetCurrentAnonymousInstance event, Emitter<ThunderState> emit) async {
    ...
    if (event.instance != null) {
      ...
      prefs.setString(
          LocalSettings.currentAnonymousInstance.name, event.instance!);
    } else {
