...
class ExtendedPostCardActions {
  ...
  final Color? Function(BuildContext context, PostView postView)?
      getForegroundColor;
  final IconData? Function(PostView postView)? getOverrideIcon;
  final String? Function(BuildContext context, PostView postView)?
      getOverrideLabel;
  final String? Function(BuildContext context, PostViewMedia postViewMedia)?
      getSubtitleLabel;
...
final List<ExtendedPostCardActions> postCardActionItems = [
  ...
  ExtendedPostCardActions(
    ...
    getSubtitleLabel: (context, postViewMedia) =>
        LemmyClient.instance.generatePostUrl(postViewMedia.postView.post.id),
  ),
  ExtendedPostCardActions(
    ...
    getSubtitleLabel: (context, postViewMedia) =>
        postViewMedia.media.first.imageUrl,
  ),
  ExtendedPostCardActions(
  ...
  ExtendedPostCardActions(
    ...
    getForegroundColor: (context, postView) => postView.myVote == 1
  ...
  ExtendedPostCardActions(
    ...
    getForegroundColor: (context, postView) => postView.saved
  ...
  ExtendedPostCardActions(
    ...
    getColor: (context) =>
        context.read<ThunderBloc>().state.markReadColor.color,
    getOverrideIcon: (postView) => postView.read
  ...
  ExtendedPostCardActions(
    ...
    getOverrideLabel: (context, postView) =>
        postView.hidden == true ? l10n.unhide : l10n.hide,
    icon: Icons.visibility_off_rounded,
    getColor: (context) => context.read<ThunderBloc>().state.hideColor.color,
    getOverrideIcon: (postView) => postView.hidden == true
  ...
  ExtendedPostCardActions(
    ...
    label: l10n.delete,
    getOverrideIcon: (postView) => postView.post.deleted
        ? Icons.restore_from_trash_rounded
        : Icons.delete_rounded,
    getOverrideLabel: (context, postView) =>
        postView.post.deleted ? l10n.restore : l10n.delete,
  ),
  ExtendedPostCardActions(
  ...
  ExtendedPostCardActions(
    ...
    icon: Icons.lock,
    label: l10n.lockPost,
    getOverrideIcon: (postView) =>
        postView.post.locked ? Icons.lock_open_rounded : Icons.lock,
    getOverrideLabel: (context, postView) =>
        postView.post.locked ? l10n.unlockPost : l10n.lockPost,
  ),
  ExtendedPostCardActions(
    postCardAction: PostCardAction.moderatorPinCommunity,
    icon: Icons.push_pin_rounded,
    label: l10n.pinToCommunity,
    getOverrideIcon: (postView) => postView.post.featuredCommunity
        ? Icons.push_pin_rounded
        : Icons.push_pin_outlined,
    getOverrideLabel: (context, postView) => postView.post.featuredCommunity
        ? l10n.unpinFromCommunity
        : l10n.pinToCommunity,
  ),
  ExtendedPostCardActions(
    ...
    getOverrideIcon: (postView) => postView.post.removed
        ? Icons.restore_from_trash_rounded
        : Icons.delete_forever_rounded,
    getOverrideLabel: (context, postView) =>
        postView.post.removed ? l10n.restorePost : l10n.removePost,
  )
];
...
void showPostActionBottomModalSheet(
  ...
  final bool isOwnPost = postViewMedia.postView.creator.id ==
      context.read<AuthBloc>().state.account?.userId;
  final bool isModerator = context.read<AccountBloc>().state.moderates.any(
      (CommunityModeratorView communityModeratorView) =>
          communityModeratorView.community.id ==
  ...
  final List<ExtendedPostCardActions> defaultPostCardActions =
      postCardActionItems
  ...
  if (isModerator) {
    defaultPostCardActions.add(postCardActionItems.firstWhere(
        (ExtendedPostCardActions extendedPostCardActions) =>
            extendedPostCardActions.postCardAction ==
            PostCardAction.moderatorActions));
  }

  // Generate the list of default multi actions
  final List<ExtendedPostCardActions> defaultMultiPostCardActions =
      postCardActionItems
          .where((extendedAction) => [
  ...
  if (defaultMultiPostCardActions.any((extendedAction) =>
          extendedAction.postCardAction == PostCardAction.hide) &&
      !LemmyClient.instance.supportsFeature(LemmyFeature.hidePosts)) {
    defaultMultiPostCardActions.removeWhere(
        (ExtendedPostCardActions postCardActionItem) =>
            postCardActionItem.postCardAction == PostCardAction.hide);
  }

  // Generate the list of moderator actions
  final List<ExtendedPostCardActions> moderatorPostCardActions =
      postCardActionItems
          .where((extendedAction) => [
  ...
  if (postViewMedia.media.isEmpty ||
      ...
      postViewMedia.media.first.originalUrl ==
          postViewMedia.media.first.mediaUrl) {
    sharePostCardActions.removeWhere((extendedAction) =>
        extendedAction.postCardAction == PostCardAction.shareLink);
  }

  // Remove the share image option if there is no image
  if (postViewMedia.media.isEmpty ||
      postViewMedia.media.first.imageUrl?.isNotEmpty != true) {
    sharePostCardActions.removeWhere((extendedAction) =>
        extendedAction.postCardAction == PostCardAction.shareImage);
  }

  // Remove the share media option if there is no media
  if (postViewMedia.media.isEmpty ||
      postViewMedia.media.first.mediaUrl?.isNotEmpty != true) {
    sharePostCardActions.removeWhere((extendedAction) =>
        extendedAction.postCardAction == PostCardAction.shareMedia);
  }

  // Remove the share local option if it is the same as the original
  if (postViewMedia.postView.post.apId ==
      LemmyClient.instance.generatePostUrl(postViewMedia.postView.post.id)) {
    sharePostCardActions.removeWhere((extendedAction) =>
        extendedAction.postCardAction == PostCardAction.sharePostLocal);
  }
  ...
  final List<ExtendedPostCardActions> userActions = postCardActionItems
      .where((extendedAction) => [
            ...
            if (postViewMedia.postView.creator.id != currentUserId)
  ...
  if (communityActions.any((extendedAction) =>
          extendedAction.postCardAction == PostCardAction.blockCommunity) &&
      postViewMedia.postView.subscribed != SubscribedType.notSubscribed) {
    communityActions.removeWhere((ExtendedPostCardActions postCardActionItem) =>
        postCardActionItem.postCardAction == PostCardAction.blockCommunity);
  }
  ...
  if (instanceActions.any((extendedAction) =>
          extendedAction.postCardAction == PostCardAction.blockInstance) &&
      !LemmyClient.instance.supportsFeature(LemmyFeature.blockInstance)) {
    instanceActions.removeWhere((ExtendedPostCardActions postCardActionItem) =>
        postCardActionItem.postCardAction == PostCardAction.blockInstance);
  }
  ...
  showModalBottomSheet<void>(
    ...
    builder: (builderContext) => PostCardActionPicker(
      ...
      multiPostCardActions: {
        PostActionBottomSheetPage.general: defaultMultiPostCardActions
      },
      titles: {
...
class PostCardActionPicker extends StatefulWidget {
  ...
  final Map<PostActionBottomSheetPage, List<ExtendedPostCardActions>>
      multiPostCardActions;
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Semantics(
                label:
                    '${widget.titles[page ?? widget.page] ?? l10n.actions}, ${(page ?? widget.page) == PostActionBottomSheetPage.general ? '' : l10n.backButton}',
                child: Padding(
                  ...
                  child: Material(
                    ...
                    child: InkWell(
                      borderRadius: BorderRadius.circular(50),
                      onTap: (page ?? widget.page) ==
                              PostActionBottomSheetPage.general
                          ? null
                          : () => setState(
                              () => page = PostActionBottomSheetPage.general),
                      child: Padding(
                        padding:
                          ...
                          child: Row(
                            children: [
                              ...
                              Semantics(
                                ...
                                child: Text(
                                  widget.titles[page ?? widget.page] ??
                                      l10n.actions,
              ...
              Row(
                children: [
                  ...
                  LanguagePostCardMetaData(
                      languageId:
                          widget.postViewMedia.postView.post.languageId),
                ],
              ),
              if (widget
                      .multiPostCardActions[page ?? widget.page]?.isNotEmpty ==
                  true)
                MultiPickerItem(
                  pickerItems: [
                    ...widget.multiPostCardActions[page ?? widget.page]!
                        .where((a) =>
                            a.shouldShow?.call(
                                context, widget.postViewMedia.postView) ??
                            true)
                        .map(
                      (a) {
                        return PickerItemData(
                          label: a.getOverrideLabel?.call(
                                  context, widget.postViewMedia.postView) ??
                              a.label,
                          icon: a.getOverrideIcon
                                  ?.call(widget.postViewMedia.postView) ??
                              a.icon,
                          backgroundColor: a.getColor?.call(context),
                          foregroundColor: a.getForegroundColor
                              ?.call(context, widget.postViewMedia.postView),
                          onSelected:
                              (a.shouldEnable?.call(isUserLoggedIn) ?? true)
                                  ? () => onSelected(a.postCardAction)
                                  : null,
                        );
                      },
                    ),
              ...
              if (widget.postCardActions[page ?? widget.page]?.isNotEmpty ==
                  true)
                ListView.builder(
                  shrinkWrap: true,
                  physics: const NeverScrollableScrollPhysics(),
                  itemCount:
                      widget.postCardActions[page ?? widget.page]!.length,
                  itemBuilder: (BuildContext itemBuilderContext, int index) {
                    return PickerItem(
                      label: widget.postCardActions[page ?? widget.page]![index]
                              .getOverrideLabel
                              ?.call(context, widget.postViewMedia.postView) ??
                          widget.postCardActions[page ?? widget.page]![index]
                              .label,
                      subtitle: widget
                          .postCardActions[page ?? widget.page]![index]
                          .getSubtitleLabel
                          ?.call(context, widget.postViewMedia),
                      icon: widget.postCardActions[page ?? widget.page]![index]
                              .getOverrideIcon
                              ?.call(widget.postViewMedia.postView) ??
                          widget.postCardActions[page ?? widget.page]![index]
                              .icon,
  ...
  void onSelected(PostCardAction postCardAction) async {
    ...
    switch (postCardAction) {
      case PostCardAction.visitCommunity:
        action = () => onTapCommunityName(
            widget.outerContext, widget.postViewMedia.postView.community.id);
        break;
      ...
      case PostCardAction.visitProfile:
        action = () => navigateToFeedPage(widget.outerContext,
            ...
            userId: widget.postViewMedia.postView.post.creatorId);
        break;
      case PostCardAction.visitInstance:
        action = () => navigateToInstancePage(widget.outerContext,
            instanceHost: fetchInstanceNameFromUrl(
                widget.postViewMedia.postView.community.actorId)!,
      ...
      case PostCardAction.sharePostLocal:
        action = () => Share.share(LemmyClient.instance
            .generatePostUrl(widget.postViewMedia.postView.post.id));
        break;
          ...
          if (widget.postViewMedia.media.first.imageUrl != null) {
            try {
              ...
              var media = await DefaultCacheManager()
                  .getFileFromCache(widget.postViewMedia.media.first.imageUrl!);
              File? mediaFile = media?.file;

              if (media == null) {
                // Tell user we're downloading the image
                showSnackbar(
                    AppLocalizations.of(widget.outerContext)!.downloadingMedia);

                // Download
                mediaFile = await DefaultCacheManager()
                    .getSingleFile(widget.postViewMedia.media.first.imageUrl!);
              }
              ...
              showSnackbar(AppLocalizations.of(widget.outerContext)!
      ...
      case PostCardAction.shareLink:
        action = () {
          if (widget.postViewMedia.media.first.originalUrl != null)
            Share.share(widget.postViewMedia.media.first.originalUrl!);
        };
        break;
      case PostCardAction.shareAdvanced:
        action = () =>
            showAdvancedShareSheet(widget.outerContext, widget.postViewMedia);
        break;
      case PostCardAction.instanceActions:
        action =
            () => setState(() => page = PostActionBottomSheetPage.instance);
        pop = false;
        break;
      case PostCardAction.blockInstance:
        action = () => widget.outerContext
            ...
            .add(InstanceActionEvent(
              ...
              instanceId: widget.postViewMedia.postView.community.instanceId,
              domain: fetchInstanceNameFromUrl(
                  widget.postViewMedia.postView.community.actorId),
              value: true,
            ));
        break;
      case PostCardAction.communityActions:
        action =
            () => setState(() => page = PostActionBottomSheetPage.community);
        pop = false;
        break;
      case PostCardAction.blockCommunity:
        action = () {
          widget.outerContext.read<CommunityBloc>().add(CommunityActionEvent(
              communityAction: CommunityAction.block,
              communityId: widget.postViewMedia.postView.community.id,
              value: true));
          widget.onBlockedCommunity
              ?.call(widget.postViewMedia.postView.community.id);
        };
        break;
      case PostCardAction.upvote:
        action = () => widget.outerContext.read<FeedBloc>().add(
            FeedItemActionedEvent(
                postAction: PostAction.vote,
                postId: widget.postViewMedia.postView.post.id,
                value: widget.postViewMedia.postView.myVote == 1 ? 0 : 1));
        break;
      ...
      case PostCardAction.save:
        action = () => widget.outerContext.read<FeedBloc>().add(
            FeedItemActionedEvent(
                ...
                postId: widget.postViewMedia.postView.post.id,
                value: !widget.postViewMedia.postView.saved));
        break;
      case PostCardAction.toggleRead:
        action = () => widget.outerContext.read<FeedBloc>().add(
            FeedItemActionedEvent(
                ...
                postId: widget.postViewMedia.postView.post.id,
                value: !widget.postViewMedia.postView.read));
        break;
      case PostCardAction.hide:
        action = () => widget.outerContext.read<FeedBloc>().add(
            FeedItemActionedEvent(
                ...
                postId: widget.postViewMedia.postView.post.id,
                value: !(widget.postViewMedia.postView.hidden ?? false)));
      ...
      case PostCardAction.blockUser:
        action = () {
          widget.outerContext.read<UserBloc>().add(UserActionEvent(
              userAction: UserAction.block,
              userId: widget.postViewMedia.postView.creator.id,
              value: true));
          widget.onBlockedCommunity
              ?.call(widget.postViewMedia.postView.creator.id);
        };
        break;
      ...
      case PostCardAction.delete:
        action = () => widget.outerContext.read<FeedBloc>().add(
            FeedItemActionedEvent(
                ...
                postId: widget.postViewMedia.postView.post.id,
                value: !widget.postViewMedia.postView.post.deleted));
        break;
      case PostCardAction.moderatorActions:
        action =
            () => setState(() => page = PostActionBottomSheetPage.moderator);
        pop = false;
        break;
      case PostCardAction.moderatorLockPost:
        action = () => widget.outerContext.read<FeedBloc>().add(
            FeedItemActionedEvent(
                postAction: PostAction.lock,
                postId: widget.postViewMedia.postView.post.id,
                value: !widget.postViewMedia.postView.post.locked));
        break;
      case PostCardAction.moderatorPinCommunity:
        action = () => widget.outerContext.read<FeedBloc>().add(
            FeedItemActionedEvent(
                postAction: PostAction.pinCommunity,
                postId: widget.postViewMedia.postView.post.id,
                value: !widget.postViewMedia.postView.post.featuredCommunity));
        break;
      case PostCardAction.moderatorRemovePost:
        action = () => showRemovePostReasonBottomSheet(
            widget.outerContext, widget.postViewMedia);
        break;
    }
...
void showRemovePostReasonBottomSheet(
    BuildContext context, PostViewMedia postViewMedia) {
  showModalBottomSheet(
    context: context,
    showDragHandle: true,
    isScrollControlled: true,
    builder: (_) => ReasonBottomSheet(
      title: postViewMedia.postView.post.removed
          ? l10n.restorePost
          : l10n.removalReason,
      submitLabel:
          postViewMedia.postView.post.removed ? l10n.restore : l10n.remove,
