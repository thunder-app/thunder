...
class AnonymousSubscriptionsBloc
    extends Bloc<AnonymousSubscriptionsEvent, AnonymousSubscriptionsState> {
  AnonymousSubscriptionsBloc() : super(const AnonymousSubscriptionsState()) {
    on<GetSubscribedCommunitiesEvent>(_getSubscribedCommunities,
  ...
  FutureOr<void> _deleteSubscriptions(DeleteSubscriptionsEvent event,
      Emitter<AnonymousSubscriptionsState> emit) async {
    try {
      await AnonymousSubscriptions.deleteCommunities(event.ids);
      emit(state.copyWith(
        status: AnonymousSubscriptionsStatus.success,
        subscriptions: [...state.subscriptions]
          ..removeWhere((e) => event.ids.contains(e.id)),
  ...
  FutureOr<void> _addSubscriptions(AddSubscriptionsEvent event,
      Emitter<AnonymousSubscriptionsState> emit) async {
    try {
  ...
  Future<void> _getSubscribedCommunities(GetSubscribedCommunitiesEvent event,
      Emitter<AnonymousSubscriptionsState> emit) async {
    emit(const AnonymousSubscriptionsState(
